...
class HashMap {
  ...
  set(key, value) {
    ...
    this.bucket[hashKey % this.bucketLength] = new Node(hashKey, value);
    this.bucketSize++;
    return;
  }
  ...
  get(key) {
    ...
    const bucketValue = this.bucket[hashKey % this.bucketLength].value;

    if (hashKey === bucketKey) return bucketValue;
    else return null;
  }
  ...
  entries() {
    ...
    for (let i = 0; i < this.bucketLength; i++) {
      if (this.bucket[i] === undefined) continue;
      entries.push([this.bucket[i].key, this.bucket[i].value]);
    }
    return entries;
  }
}
